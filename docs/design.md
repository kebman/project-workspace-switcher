...
Create or remove worktrees with Git, then update `PWS_PATHS` so the switcher knows where each workspace and repo/path slot lives. The registry commands help with that config update, but still do not call `git worktree add`, `git worktree remove` or delete local directories.

## Managed Registry Block

Registry commands such as `pws add`, `pws rm`, `pws path set` and `pws path rm` update the config file pointed to by `PWS_CONFIG_FILE`. They do this by replacing or appending a generated block:
...
The engine deliberately avoids parsing arbitrary Zsh config. It sources the current config to get the registry values, updates those in memory, then serialises only the managed registry variables back into the generated block. If no block exists, the block is appended to the end of the config so existing hand-written settings remain in place.

`PWS_REPOS` and `PWS_REPO_LABELS` stay hand-edited because they define the stable repo/path slots for a user's setup. Aliases and other shell customisations should also stay outside the managed block.

After a registry command writes the config, the engine reloads it in the current shell session so `pws ls`, `pws <workspace>`, `pcd`, `pg` and `pag` see the updated data immediately.
...
The repo-local smoke test validates engine syntax, example config syntax, basic workspace selection behaviour and registry command updates against a temporary config file. It does not validate real filesystem path existence in the examples, because example paths are placeholders by design.
